"""Shared pytest fixtures + import shims for the AutoGen adapter test suite.

`adapter.py` imports at module load:
  - molecule_runtime.adapters.base (BaseAdapter, AdapterConfig)
  - molecule_runtime.adapters.shared_runtime (build_task_text, brief_task,
    extract_history, extract_message_text, set_current_task)
...
def _install_stubs() -> None:
    ...
    if "molecule_runtime" not in sys.modules:
        ...
        mr.adapters.shared_runtime = types.ModuleType(
            "molecule_runtime.adapters.shared_runtime"
        )
        mr.adapters.shared_runtime.build_task_text = lambda *a, **k: ""
        mr.adapters.shared_runtime.brief_task = lambda *a, **k: ""
        mr.adapters.shared_runtime.extract_history = lambda *a, **k: []
        mr.adapters.shared_runtime.extract_message_text = lambda *a, **k: ""
